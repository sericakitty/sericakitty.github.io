	...
	export default class LevelHandler extends Scene {
		...
		createEnemies() {
			...
			for (let yVal = 1; yVal <= this.gameState.numOfEnemiesPerYLine; yVal += 1) {
				for (let xVal = 1; xVal < this.gameState.numOfEnemiesPerXLine * 2; xVal += 1.5) {
					...
					this.gameState.enemies.name = this.gameState.enemyGroups[enemyIndex];
				}
			}

			this.gameState.enemies.getChildren().forEach((bug) => {
				bug.name = this.gameState.enemies.name;
		...
		levelWin() {
			if (this.numOfTotalEnemies() === 0) {
				...
				this.add.text(this.width * 0.5, this.height * 0.55, `Click to start ${this.nextLevel[this.levelKey]}`, { fontSize: '25px', fill: '#000000' }).setOrigin();
				this.goNextLevel();
			}
		}
		...
		goNextLevel() {
			this.input.on('pointerup', () => {
				...
				console.log(this.nextLevel[this.levelKey]);
			});
		}

		launchScene(scene, key) {
			this.scene.stop(key);
		...
		create() {
			...
			this.destroyEnemy();
			this.gameOver();
			// this.launchScene('bootloader', this);
		}
